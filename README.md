...
**Текущая версия: v1.5.6.**

## Что нового в v1.5.6

- **`ocvpn --update`** — скачивает и ставит последнюю версию из GitHub (смотрит
  release/теги/main, берёт максимальную). Плюс **автопроверка при запуске**: если
  есть новее — в терминале предложит обновиться и перезапустится новой версией.
  Отключение: `OCVPN_NO_UPDATE_CHECK=1`.
- **Исправления подписок/сети**: base64-подписки больше не теряются (SIGPIPE под
  `pipefail`); нормализация CRLF/отступов; нечитаемая `/etc/ocvpn/subs-url` не
  роняет скрипт; **пропуск РФ-нод** (флаг 🇷🇺, `RU`/`RUS`/`RUSSIA`/`RF`,
  кириллица `РУ`/`РУС`/`РФ`, города; `OCVPN_SKIP_RU=0` отключает).
- **Надёжность macOS**: автоподъём прав из терминала (`--user-home` — root
  работает с логом opencode/auth/своими хостами пользователя), `HOME`-фолбэк
  `/var/root`, `--daemon --watch` снова поднимает и держателя, и вотчдог,
  безопасное убийство процессов (не убивает себя/предка под `sudo`).
- Покрытие тестами ≥95% (новые кластеры `cov-hardening`, `cov-update`).
